  ...
  Future<void> _refresh({bool showErrorSnackBar = true}) async {
    ...
    result.fold(
      (failure) => ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(failure.message ?? 'รีเฟรชบัญชีไม่สำเร็จ')),
      ),
      (_) {},
    );
  }
            ...
            Expanded(
              child: RefreshIndicator(
                ...
                child: accountsAsync.when(
                  ...
                  error: (error, _) =>
                      Center(child: Text('โหลดบัญชีไม่สำเร็จ: $error')),
                  data: (accounts) {
                    if (accounts.isEmpty) {
                      return ListView(
                        children: const [
                          Padding(
                            ...
                            child: Center(
                              child: Text(
                                'ยังไม่มีบัญชี — แตะ + เพื่อเพิ่มบัญชี',
                              ),
                            ),
                          ),
                        ],
...
class _NetWorthHero extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    for (final account in accounts) {
      final balance =
          ref.watch(accountCurrentBalanceProvider(account.id)).value ?? 0;
      if (balance < 0) {
        debts += -balance;
      } else {
        assets += balance;
