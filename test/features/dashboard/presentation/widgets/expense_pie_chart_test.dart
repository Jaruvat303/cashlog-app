...
void main() {
  testWidgets('every slice gets its own floating "%" label, including ones under 10% share', (tester) async {
  ...
  testWidgets('the chart is significantly bigger than the old fixed 110px size', (tester) async {
    ...
    final chart = tester.widget<SizedBox>(find.byKey(const Key('pieChartCircle')));
    expect(chart.width, greaterThan(160), reason: 'a merely-marginal size bump would not satisfy "ใหญ่ขึ้นชัดเจน"');
    expect(chart.height, chart.width);
  });

  testWidgets('a category sitting exactly at 10% share still gets its own label (no cutoff anymore)', (tester) async {
  ...
  testWidgets('percentage labels sit outside the ring, at increasing angular offsets from the top', (tester) async {
    ...
    final breakdown = [_breakdown(1, 'A', 250), _breakdown(2, 'B', 250), _breakdown(3, 'C', 250), _breakdown(4, 'D', 250)];

    await tester.pumpWidget(
      MaterialApp(home: Scaffold(body: ExpensePieChart(expense: breakdown, total: 1000))),
    );

    final chartCenter = tester.getCenter(find.byKey(const Key('pieChartCircle')));
    final chartSize = tester.getSize(find.byKey(const Key('pieChartCircle'))).width;
    final centers = [
      for (var id = 1; id <= 4; id++) tester.getCenter(find.byKey(Key('pieChartPercentLabel-$id')))
    ];

    // Every label sits outside the visible ring...
    for (final labelCenter in centers) {
      expect((labelCenter - chartCenter).distance, greaterThan(chartSize / 2));
    }
