...
class _FakeAccountsRepository implements AccountsRepository {
  ...
  Stream<double> watchCurrentBalance(int accountId) =>
      Stream.value(balances[accountId] ?? 0);
...
class _MonthChannel {
  ...
  Stream<List<Transaction>> streamFor({
    int? categoryId,
...
class _FakeTransactionsRepository implements TransactionsRepository {
  ...
  _MonthChannel _channelFor(int year, int month) =>
...
Transaction _transfer(int id, double amount, DateTime date) => Transaction(
  id: id,
  amount: amount,
...
Future<void> _scrollToFinder(WidgetTester tester, Finder finder) => tester
...
bool _categoryRowIsHighlighted(WidgetTester tester, int categoryId) {
  final container = tester.widget<Container>(
    find
        .descendant(
          of: find.byKey(Key('categoryTotalRow-$categoryId')),
...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final pageOneRows = List.generate(
        20,
        (i) => _expense(i + 1, 'Page one row $i', thisMonth),
      );
  ...
  testWidgets('switching month replaces the list with the new month\'s data', (
    tester,
  ) async {
  ...
  group('T13 stuck-items badge', () {
    testWidgets('hidden when the retry queue is empty', (tester) async {
      ...
      final badge = tester.widget<Badge>(
        find.descendant(
          of: find.byKey(const Key('pendingActionsButton')),
    ...
    testWidgets('shows the queue count and opens the stuck-items page on tap', (
      tester,
    ) async {
  ...
  group('summary tabs (ticket 07) and category drill-through (ticket 10)', () {
    setUp(() {
      fakeDashboard.results[(thisMonth.year, thisMonth.month)] = Right(
        DashboardSummary(
          ...
          expense: [
            _breakdown(10, 'อาหาร', 2000),
            _breakdown(11, 'เดินทาง', 500),
          ],
        ),
      );
    });
    ...
    testWidgets(
      'defaults to the Expense tab, listing categories with their totals as text',
      (tester) async {
        await tester.pumpWidget(buildApp());
        await _pumpBounded(tester);
        ...
        expect(
          find.descendant(
            of: find.byKey(const Key('categoryTotalRow-10')),
            matching: find.text('อาหาร'),
          ),
          findsOneWidget,
        );
        expect(
          find.descendant(
            of: find.byKey(const Key('categoryTotalRow-11')),
            matching: find.text('เดินทาง'),
          ),
          findsOneWidget,
        );
        ...
        expect(
          find.text('เงินเดือน'),
          findsNothing,
        ); // income row, not shown while Expense tab is active
        expect(
          find.byType(PieChart),
          findsOneWidget,
        ); // ticket 03: pie chart restored above the list
      },
    );

    testWidgets(
      'switching to the Income tab shows income categories and totals instead',
      (tester) async {
        await tester.pumpWidget(buildApp());
        await _pumpBounded(tester);
        ...
        expect(
          find.descendant(
            of: find.byKey(const Key('categoryTotalRow-20')),
            matching: find.text('เงินเดือน'),
          ),
          findsOneWidget,
        );
    ...
    group('ticket 10: category drill-through', () {
      testWidgets(
        'tapping a category row filters the day-grouped list below to only that category',
        (tester) async {
          fakeTransactions.pages[(
            thisMonth.year,
            thisMonth.month,
      ...
      testWidgets(
        'tapping the same category row again clears the filter, restoring the full list',
        (tester) async {
          fakeTransactions.pages[(
            thisMonth.year,
            thisMonth.month,
      ...
      testWidgets(
        'the selected category row is visually highlighted; tapping it off removes the highlight',
        (tester) async {
          await tester.pumpWidget(buildApp());
          await _pumpBounded(tester);
      ...
      testWidgets(
        'tapping a different category while one is already selected moves the highlight and the filter',
        (tester) async {
          fakeTransactions.pages[(
            thisMonth.year,
            thisMonth.month,
      ...
      testWidgets('switching months clears an active category filter', (
        tester,
      ) async {
        final nextMonth = DateTime.utc(thisMonth.year, thisMonth.month + 1);
      ...
      testWidgets(
        'switching the summary tab clears an active category filter',
        (tester) async {
          fakeTransactions.pages[(
            thisMonth.year,
            thisMonth.month,
  ...
  group('ticket 12: "Uncategorized" drill-through', () {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await _pumpBounded(tester);
        await tester.tap(
          find.byKey(Key('categoryTotalRow-$kUncategorizedCategoryId')),
        );
        await _pumpBounded(tester);
        await _scrollToFinder(tester, find.text('Uncategorized expense row'));
    ...
    testWidgets(
      'the Uncategorized row toggles off like any other category row, restoring the full list',
      (tester) async {
        fakeTransactions.pages[(
          thisMonth.year,
          thisMonth.month,
    ...
    testWidgets(
      ...
      (tester) async {
        fakeTransactions.pages[(
          ...
          transactions: [
            _expense(1, 'Uncategorized expense row', thisMonth),
            _income(2, 'Uncategorized income row', thisMonth),
          ],
          currentPage: 1,
          totalPages: 1,
        );
  ...
  group('ticket 08: Transfer summary tab', () {
    ...
    testWidgets(
      'shows every transfer transaction for the month, flat and ungrouped',
      (tester) async {
        fakeTransactions.pages[(
          thisMonth.year,
          thisMonth.month,
        ...
        expect(
          find.descendant(
            of: transferList,
            matching: find.text(formatAmount(1234)),
          ),
          findsOneWidget,
        );
        expect(
          find.descendant(
            of: transferList,
            matching: find.text(formatAmount(5678)),
          ),
          findsOneWidget,
        );
        ...
        expect(
          find.descendant(
            of: transferList,
            matching: find.text('Food expense row'),
          ),
          findsNothing,
        );
        ...
        expect(
          list.children.whereType<Divider>().length,
          1,
        ); // 2 rows => 1 separating divider
      },
    );

    testWidgets(
      'shows a "no transfers" empty state distinct from the placeholder copy',
      (tester) async {
        fakeTransactions.pages[(thisMonth.year, thisMonth.month, 1)] =
    ...
    testWidgets(
      'switching between Expense and Transfer tabs never empties either one',
      (tester) async {
        fakeDashboard.results[(thisMonth.year, thisMonth.month)] = Right(
          DashboardSummary(
            totalIncome: 0,
        ...
        expect(
          find.descendant(
            of: find.byKey(const Key('transferTabList')),
            matching: find.text(formatAmount(999)),
          ),
          findsOneWidget,
        );
  ...
  group('ticket 04: summary page consolidation', () {
    ...
    testWidgets(
      'the page-level "+" button is gone; the pending-actions button remains',
      (tester) async {
        await tester.pumpWidget(buildApp());
        await _pumpBounded(tester);
    ...
    testWidgets(
      'the account-info strip renders on this page, showing each account and its balance',
      (tester) async {
        await tester.pumpWidget(
          buildApp(
            accountsRepository: _FakeAccountsRepository(
              accounts: const [account],
              balances: const {1: 2500},
            ),
          ),
        );
        await _pumpBounded(tester);
        ...
        expect(
          find.descendant(
            of: find.byKey(const Key('accountInfoStrip')),
        ...
        expect(
          find.descendant(
            of: find.byKey(const Key('accountInfoStrip')),
            matching: find.text(formatAmount(2500)),
          ),
          findsOneWidget,
        );
      },
    ...
    group('post-launch-polish-03: Account Card subtitle', () {
      testWidgets(
        'shows the account\'s matching_keywords as a white subtitle, not the generic balance disclaimer',
        (tester) async {
          ...
          await tester.pumpWidget(
            buildApp(
              accountsRepository: _FakeAccountsRepository(
                accounts: const [withKeywords],
                balances: const {2: 1000},
              ),
            ),
          );
          await _pumpBounded(tester);
      ...
      testWidgets(
        'an account with no matching_keywords shows a plain placeholder, still not the disclaimer',
        (tester) async {
          await tester.pumpWidget(
            buildApp(
              accountsRepository: _FakeAccountsRepository(
    ...
    testWidgets(
      'the month switcher lives in the AppBar title, not a secondary row below it',
      (tester) async {
        await tester.pumpWidget(buildApp());
        await _pumpBounded(tester);
        ...
        expect(
          ...
          isNotNull,
          reason:
              'the month switcher should be the AppBar title, matching Home',
        );
        expect(
          find.descendant(
            of: find.byWidget(appBar.title!),
            matching: find.text(monthYearShortLabel(thisMonth)),
          ),
          findsOneWidget,
        );
    ...
    testWidgets(
      'switching months updates the account info, the summary tabs, and the list below together',
      (tester) async {
        final nextMonth = DateTime.utc(thisMonth.year, thisMonth.month + 1);
        fakeDashboard.results[(thisMonth.year, thisMonth.month)] = Right(
          DashboardSummary(
        ...
        fakeTransactions.pages[(
          ...
          transactions: [
            _expense(1, 'This month row', thisMonth, categoryId: 10),
          ],
          currentPage: 1,
          totalPages: 1,
        );
        fakeTransactions.pages[(
          ...
          transactions: [
            _expense(2, 'Next month row', nextMonth, categoryId: 11),
          ],
          currentPage: 1,
          totalPages: 1,
        );
