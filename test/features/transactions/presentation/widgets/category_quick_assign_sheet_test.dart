...
void main() {
  ...
  setUp(() {
    ...
    fakeCategories = _FakeCategoriesRepository()
      ..categories = const [_groceries, _transport, _salary];
    cacheInvalidator = _RecordingCacheInvalidator();
  });

  Widget buildApp(
    Transaction transaction, {
    Map<int, Category> categoriesById = const {},
  }) => ProviderScope(
    overrides: [
  ...
  testWidgets(
    'tapping the chip opens the sheet with only categories matching the transaction type, plus Uncategorized',
    (tester) async {
      ...
      expect(
        find.byKey(const Key('categoryOptionUncategorized')),
        findsOneWidget,
      );
  ...
  testWidgets(
    'picking a category PATCHes immediately and invalidates the transaction month on success',
    (tester) async {
  ...
  testWidgets(
    'a failed PATCH shows an error and never invalidates any cache (no optimistic update left dangling)',
    (tester) async {
      fakeTransactions.updateResult = (id, categoryId) =>
          const Left(UnknownFailure(message: 'Could not update category'));
      await tester.pumpWidget(buildApp(_expenseTransaction));
      await _pumpBounded(tester);
  ...
  testWidgets(
    're-picking the transaction\'s current category is a no-op — no PATCH, no cache invalidation',
    (tester) async {
      ...
      await tester.pumpWidget(
        buildApp(alreadyGroceries, categoriesById: const {10: _groceries}),
      );
      await _pumpBounded(tester);
      ...
      expect(
        find.descendant(
          of: find.byKey(const Key('categoryOption_10')),
  ...
  testWidgets(
    'an uncategorized transaction shows the "ยังไม่ระบุหมวดหมู่" badge',
    (tester) async {
      await tester.pumpWidget(buildApp(_expenseTransaction));
      await _pumpBounded(tester);
