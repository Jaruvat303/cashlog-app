    ...
    home: Scaffold(
      body: Builder(
        builder: (context) => ElevatedButton(
          onPressed: () async {
            _lastResult = await showCategoryGridPicker(
              context,
...
Finder _sheetSizedBox() =>
    find.byWidgetPredicate((w) => w is SizedBox && (w.height ?? 0) > 100);
...
void main() {
  ...
  testWidgets(
    'sheet height is the same half-screen size for a short category list as for a long one',
    (tester) async {
  ...
  testWidgets(
    'a category list too tall to fit shows a visible scrollbar and is reachable by scrolling',
    (tester) async {
  ...
  testWidgets(
    'a category list that fits within half-screen needs no scrolling and selecting still closes the sheet',
    (tester) async {
      await tester.pumpWidget(_buildApp(_expenseCategories(2)));
      await tester.tap(find.text('open'));
      await _pumpBounded(tester);
  ...
  group('ticket 11: CategoryGridTile overflow regression', () {
    testWidgets(
      'a long, 2-line category name does not overflow its grid tile at phone width',
      (tester) async {
        ...
        final categories = [
          ...
          Category(
            id: 1,
            name: 'ที่อยู่อาศัย (ค่าเช่า/ผ่อนบ้าน)',
            type: CategoryType.expense,
            iconKey: 'home-4-fill',
            colorHex: '#FF6B6B',
          ),
          Category(
            id: 2,
            name: 'เบี้ยประกัน (ชีวิต/สุขภาพ/รถ)',
            type: CategoryType.expense,
            iconKey: 'shield-check-fill',
            colorHex: '#FF6B6B',
          ),
        ];
