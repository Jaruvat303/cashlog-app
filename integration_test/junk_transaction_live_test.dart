...
void main() {
  ...
  testWidgets('T12: a non-slip gallery image uploads as junk on the real backend, the real feed '
      "shows the badge, deleting hits DELETE /transactions/:id for real, and editing "
      'through T6\'s real form clears isJunk', (tester) async {
    ...
    // ignore: avoid_print
    print(
      'T12-LAYER2: accounts refresh -> ${refreshResult.fold((f) => 'FAILED: $f', (_) => 'ok')}',
    );
    ...
    // ignore: avoid_print
    print(
      'T12-LAYER2: ${accounts.length} active account(s): ${accounts.map((a) => '${a.id}:${a.name}').join(', ')}',
    );
    expect(
      accounts,
      isNotEmpty,
      reason: "need at least one active dev account to drive T6's edit form",
    );
    ...
    expect(
      access,
      isNot(GalleryAccessLevel.denied),
      reason: 'gallery access must not be denied for the scan to find anything',
    );
    ...
    for (var i = 0; i < 5; i++) {
      ...
      // ignore: avoid_print
      print(
        'T12-LAYER2: warm-up album query (attempt $i) found ${warmupCandidates.length} candidate(s)',
      );
    ...
    expect(
      ...
      isNotEmpty,
      reason:
          'expected the pushed gallery images to be visible via photo_manager',
    );
    ...
    for (var i = 0; i < 4; i++) {
      ...
      // ignore: avoid_print
      print(
        'T12-LAYER2: scan pass attempt $i: total=${progress.total} completed=${progress.completed}',
      );
      for (final r in progress.results) {
        // ignore: avoid_print
        print(
          'T12-LAYER2:   ${r.filename} -> ${r.status}${r.failureMessage != null ? ' (${r.failureMessage})' : ''}',
        );
      }
    ...
    for (var guard = 0; guard < 24; guard++) {
      ...
      if (DateTime.utc(
        current.year,
        current.month,
      ).isBefore(DateTime.utc(now.year, now.month))) {
        monthNotifier.next();
      } else {
    ...
    final syncResult = await repo.fetchPage(
      year: now.year,
      month: now.month,
      page: 1,
      limit: 100,
    ...
    final junkRows = rows.where((r) => r.isJunk).toList()
      ..sort((a, b) => a.id.compareTo(b.id));
    // ignore: avoid_print
    print(
      'T12-LAYER2: ${junkRows.length} junk row(s) out of ${rows.length} total: ${junkRows.map((r) => r.id).join(', ')}',
    );
    expect(
      junkRows,
      isNotEmpty,
    ...
    for (final row in junkRows) {
      expect(
        row.amount,
        0,
        reason: '${row.localImageName} (id ${row.id}) should have amount==0',
      );
      expect(
        row.senderName,
        isEmpty,
        reason:
            '${row.localImageName} (id ${row.id}) should have an empty senderName',
      );
      expect(
        row.receiverName,
        isEmpty,
        reason:
            '${row.localImageName} (id ${row.id}) should have an empty receiverName',
      );
    }
    ...
    final badgeCountBefore = find
        .text("Couldn't read slip data")
        .evaluate()
        .length;
    // ignore: avoid_print
    print(
      'T12-LAYER2: warning badge count in the live-rendered feed = $badgeCountBefore',
    );
    expect(
      badgeCountBefore,
      greaterThanOrEqualTo(1),
      reason: 'the real feed should render the warning badge for the real junk transaction',
    );
    ...
    final junkIdsAfterDelete = (await db.select(db.cachedTransactions).get())
        .where((r) => r.isJunk)
        .map((r) => r.id)
        .toSet();
    ...
    expect(
      deletedIds,
      hasLength(1),
      reason: 'exactly one junk row should have been removed locally by the delete action',
    );
    ...
    // ignore: avoid_print
    print(
      'T12-LAYER2: id $deletedId still present server-side after delete? $stillThereServerSide',
    );
    expect(
      stillThereServerSide,
      isFalse,
      reason: 'DELETE must have hit the real backend, not just the local cache',
    );
    ...
    final remainingJunk = allRowsAfterSync
        .where((r) => r.isJunk && r.id != rowToDelete.id)
        .toList();
    // ignore: avoid_print
    print(
      'T12-LAYER2: ${remainingJunk.length} junk row(s) available to edit after sync: ${remainingJunk.map((r) => r.id).join(', ')}',
    );
    if (remainingJunk.isEmpty) {
      // ignore: avoid_print
      print(
        'T12-LAYER2: no other junk row available — edit-clears-isJunk step skipped '
    ...
    final openedTransaction = tester
        .widget<TransactionFormPage>(formFinder)
        .initial;
    final rowToEditId = openedTransaction.id;
    final isTransfer = openedTransaction.type == TransactionType.transfer;
    // ignore: avoid_print
    print(
      'T12-LAYER2: editing junk row id=$rowToEditId type=${openedTransaction.type}',
    );
    if (isTransfer && accounts.length < 2) {
      // ignore: avoid_print
      print(
        'T12-LAYER2: the opened junk row is a transfer but fewer than 2 active accounts exist — edit step skipped.',
      );
      return;
    }
    ...
    final editedRow = await (db.select(
      db.cachedTransactions,
    )..where((t) => t.id.equals(rowToEditId))).getSingleOrNull();
    // ignore: avoid_print
    print(
      'T12-LAYER2: after edit, row $rowToEditId -> amount=${editedRow?.amount} isJunk=${editedRow?.isJunk}',
    );
    expect(
      editedRow,
      isNotNull,
      reason: 'the edited row should still exist after a successful PATCH',
    );
    expect(
      editedRow!.amount,
      500,
      reason: 'the real PATCH response should reflect the new amount',
    );
    expect(
      editedRow.isJunk,
      isFalse,
      reason: 'a nonzero amount from a real PATCH response should clear isJunk',
    );
    ...
    final badgeCountAfter = find
        .text("Couldn't read slip data")
        .evaluate()
        .length;
    // ignore: avoid_print
    print(
      'T12-LAYER2: warning badge count in the live feed after edit = $badgeCountAfter (was $badgeCountBefore)',
    );
    expect(
      badgeCountAfter,
      lessThan(badgeCountBefore),
      reason:
          'the edited row\'s badge should have disappeared from the real feed',
    );
